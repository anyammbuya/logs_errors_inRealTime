...
const generateEmailContent = (data, message) => {
  ...
  return {
    Destination: { ToAddresses: ["yoshijoma@gmail.com"] },
    Message: {
      Body: {
    ...
    Source: "yoshijoma@gmail.com",
    SourceArn: "arn:aws:ses:us-east-1:058264198774:identity/yoshijoma@gmail.com",
    Tags: [{ Name: "sender", Value: "yoshi" }],
  };
};

const getLogsAndSendEmail = async (message, metricFilterData) => {
  ...
  try {
    const logsResponse = await cwlClient.send(new FilterLogEventsCommand(parameters));
    const emailContent = generateEmailContent(logsResponse, message);
    const emailResponse = await sesClient.send(new SendEmailCommand(emailContent));
...
export const dispatchErrors = async (event, context) => {
  ...
  try {
    const metricFiltersResponse = await cwlClient.send(
      new DescribeMetricFiltersCommand(requestParams)
    );
    await getLogsAndSendEmail(message, metricFiltersResponse);
  } catch (err) {
    console.error("Error occurred:", err);
  }
};
